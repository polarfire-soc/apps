# Error Correcting Code for L2 LIM
...
In the Libero design, L2 Cache is configured as L2 LIM with 8 ways starting from way 15. The L2 LIM can be accessed from 0x0800_0000. For more information, see the ICICLE KIT reference design. MSS MMUART_0 is used in the design for serial communication between the user and the application.
...
After the device is programmed, power cycle the board. To run the application, follow:
...
8. Type 2 to get the updated ECC Fix count register value. The value is incremented by 1. 

This indicates that the data is corrected after error injection and recovered.
